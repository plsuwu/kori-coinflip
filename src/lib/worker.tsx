...
export const revoke = async (token: string) => {
    ...
    if (revoked.status !== 200) {
        const err = await revoked.json();
        console.log('[-] There was an issue revoking an access token:', await err);

        return;
    }

    const res = await revoked.json();
    console.log('[+] Successfully revoked access token: ', await res);
    return;
};
